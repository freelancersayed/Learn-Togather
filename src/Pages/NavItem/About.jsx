import React from 'react';
import Navbar from '../Shared/Navbar';
import Modal from '../StudySection/Modal';
import 'animate.css';

const About = () => {
    return (
        <div>
          <Navbar></Navbar>
          <Modal></Modal> 
          <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl w-full space-y-8">
        <div>
          <h2 className="mt-6 text-center text-3xl leading-9 font-extrabold text-gray-900 animate__animated  animate__fadeInUp">
            About <span className='text-red-500'> Us</span>
          </h2>
          
          <p className="mt-2 text-center text-sm leading-5 text-gray-600">
            Learn more about our company and values
          </p>
        </div>
        <div className="bg-white shadow overflow-hidden sm:rounded-lg" data-aos="zoom-in-right">
          <div className="px-4 py-5 sm:px-6">
            <h3 className="text-lg leading-6 font-medium text-gray-900">
              Welcome to the Education Collaboration Platform!
            </h3>
            <p className="mt-1 max-w-2xl text-sm leading-5 text-gray-500">
              This web application is designed to make education more accessible and convenient. Teachers can create both free and paid sessions, and students can explore, book, and attend these sessions. Admins have the ability to set session prices, and students can view teacher profiles, rate sessions, and leave comments.
            </p>
          </div>
          <div className="border-t border-gray-200">
            <dl>
              <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                <dt className="text-sm leading-5 font-medium text-gray-500">
                  Company
                </dt>
                <dd className="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                  Fs Institute
                </dd>
              </div>
              <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                <dt className="text-sm leading-5 font-medium text-gray-500">
                  Founded
                </dt>
                <dd className="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                  2024
                </dd>
              </div>
              <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                <dt className="text-sm leading-5 font-medium text-gray-500">
                  Headquarters
                </dt>
                <dd className="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                  Dhaka, Bangladesh
                </dd>
              </div>
              <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                <dt className="text-sm leading-5 font-medium text-gray-500">
                  Employees
                </dt>
                <dd className="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                  50+
                </dd>
              </div>
              <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                <dt className="text-sm leading-5 font-medium text-gray-500">
                  Email
                </dt>
                <dd className="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                  freelancersayed45@gamil.com
                </dd>
              </div>
              <div className="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                <dt className="text-sm leading-5 font-medium text-gray-500">
                  Phone
                </dt>
                <dd className="mt-1 text-sm leading-5 text-gray-900 sm:mt-0 sm:col-span-2">
                  +880 1908-945145
                </dd>
              </div>
            </dl>
          </div>
        </div>
      </div>
    </div>
        </div>
    );
};

export default About;<h1 className="4xl">About Us</h1>